...
export class AgentLoader extends System {
    ...
    resolveUrl(url) {
        ...
        if (url.startsWith("asset://")) {
            const worldProps = this.world;
            if (!worldProps.assetsUrl) {
                console.error("[AgentLoader] Cannot resolve asset:// URL, world.assetsUrl not set.");
                return null;
            }
    ...
    async load(type, url) {
        ...
        const promise = fetch(resolved)
            .then(async (response) => {
            if (!response.ok) {
                throw new Error(`[AgentLoader] HTTP error ${response.status} for ${resolved}`);
            }
            ...
            if (type === 'script') {
                ...
                const isForbidden = forbiddenTypes.some(type => new RegExp(`app\\.create\\s*\\(\\s*['"]${type}['"]\\s*(,|\\))`).test(code));
                if (isForbidden) {
            ...
            console.error(`[AgentLoader] Failed to load ${type} from ${resolved}`, error);
            throw error;
        });
        ...
        return new Promise((resolve, reject) => {
            this.gltfLoader.parse(arrayBuffer, "", (gltf) => {
                ...
                if (type === "model") {
                    ...
                    const worldProps = this.world;
                    const setupMaterial = worldProps.setupMaterial || ((material) => {
                    ...
                    return reject(new Error(`[AgentLoader] Unsupported GLTF type: ${type}`));
                }
