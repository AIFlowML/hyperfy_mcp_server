...
function processStopReason(providedReason) {
    ...
    const isUrgent = urgentKeywords.some(keyword => cleanReason.toLowerCase().includes(keyword));
...
function detectCurrentActivity(controls) {
    try {
        // Check if we have method to detect navigation state
        if (typeof controls.getIsNavigating === 'function') {
            if (controls.getIsNavigating())
                return 'navigation';
        }
        // Check if we have method to detect patrol state  
        if (typeof controls.getIsPatrolling === 'function') {
            if (controls.getIsPatrolling())
                return 'patrol';
        }
        return 'unknown';
    }
...
//# sourceMappingURL=stopTool.js.map