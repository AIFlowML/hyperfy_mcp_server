...
function getFirstAvailableField(obj: Record<string, unknown>, fields: string[]): string | null {
...
function checkForRecentDuplicateContent(
  sessionHistory: Array<{ timestamp: number; message: string; thought?: string }>, 
  proposedContent: string,
  maxAgeMs = 30000 // 30 seconds
): { isDuplicate: boolean; recentEntry?: { timestamp: number; message: string; thought?: string } } {
  const now = Date.now();
  const recentDuplicate = sessionHistory.find(entry => 
    entry.message === proposedContent && (now - entry.timestamp) < maxAgeMs
  );
...
The agent will use current world context, recent interactions, and environmental observations to generate appropriate ambient remarks.`,
  ...
  execute: async (
    args: AmbientSpeechParams & { context?: string; agentName?: string; forceGenerate?: boolean },
    context: {
      log: LogType,
    ...
    try {
      ...
      const providedContent = getFirstAvailableField(args as unknown as Record<string, unknown>, ['content', 'message', 'text']);
      ...
      if (providedContent) {
        ...
        const duplicateCheck = checkForRecentDuplicateContent(
          sessionData.ambientHistory || [], 
          ambientContent
        );
        ...
        if (world?.entities?.items) {
          try {
            ...
            const entityDescriptions = entities.map((entry: unknown) => {
              const [id, entity] = entry as [string, HyperfyEntity];
              const typedEntity = entity as HyperfyEntity;
