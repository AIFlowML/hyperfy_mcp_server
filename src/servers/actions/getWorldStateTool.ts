...
The tool returns both structured data for programmatic use and formatted text for AI consumption. This comprehensive world state enables intelligent agent behavior and contextual decision making.`,
  ...
  execute: async (
    ...
    try {
      ...
      const service = session.data.hyperfyService as HyperfyService;
      ...
      if (entities) {
        for (const [id, entity] of entities.entries()) {
          ...
          let positionData: { x: number; y: number; z: number } | null = null;
          let posStr = 'N/A';
      ...
      let animationText = '';
      let emotesList: Array<{ name: string; description: string }> = [];
