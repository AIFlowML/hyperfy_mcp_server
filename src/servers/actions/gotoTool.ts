...
import { z } from 'zod';
import type { ActionResult, LogType, GotoSessionData } from '../../types/index.js';
...
const entityExtractionTemplate = (thoughts?: string) => {
  return `
# Task:
...
function extractThoughtSnippets(context: string): string {
...
Agent: "Located mysterious orb, navigating there..."`,
  ...
  execute: async (
    args: { entityId?: string; targetId?: string; targetDescription?: string; extractionContext?: string; forceExtraction?: boolean },
    context: {
      log: LogType,
    ...
    const sessionData = session.data as GotoSessionData;
    ...
    try {
      ...
      let targetEntityId = getFirstAvailableField(args as Record<string, unknown>, ['entityId', 'targetId']);
      ...
      if ((!targetEntityId || forceExtraction) && (targetDescription || extractionContext)) {
        ...
        try {
          ...
          const worldEntityIds = Array.from(world.entities.items.keys()) as string[];
          ...
          const entityDescriptions = limitedEntityIds.map((id) => {
            ...
            const position = (entity as { base?: { position?: { x: number; z: number } }; root?: { position?: { x: number; z: number } } })?.base?.position || (entity as { base?: { position?: { x: number; z: number } }; root?: { position?: { x: number; z: number } } })?.root?.position;
